...
public class EspaiPublicDao
{
    ...
    public void addEspaiPublic(EspaiPublic espaiPublic) {
        jdbcTemplate.update("INSERT INTO espaiPublic VALUES(?, ?, ?, ?, ?, CAST(? AS tipus_sol), CAST(? AS tipus_access), ?, ?, ?, ?, ?)",
                espaiPublic.getId(), espaiPublic.getIdMunicipi(), espaiPublic.getNom(), espaiPublic.getNomMunicipi(),
    ...
    public void updateEspaiPublic(EspaiPublic espaiPublic) {
        jdbcTemplate.update("UPDATE espaiPublic SET id_municipi = ?, nom = ?, nom_municipi = ?, tipus = ?, terreny = CAST(? AS tipus_sol), tipus_acces = CAST(? AS tipus_access), localitzacio = ?, longitud = ? , amplaria = ?, imagen = ?, descripcio = ? WHERE id = ?",
                espaiPublic.getIdMunicipi(), espaiPublic.getNom(), espaiPublic.getNomMunicipi(), espaiPublic.getTipus(),
